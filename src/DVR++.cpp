...
int DVR::AddHermBasis(int mode, int N,double Freq, double Delta, double Xeq) {
  if ( (mode>=Dim) && (mode<0) ) { return EXIT_FAILURE; }
  if (Basis[mode] != 0 ) { return EXIT_FAILURE; }
  NGrid[mode] = N;
  Basis[mode] = new BasisHerm();
  Basis[mode]->SetFreq(Freq);
  Basis[mode]->SetBasisSet(N,Delta,Xeq);
  return EXIT_SUCCESS;
}

int DVR::AddSineBasis(int mode, int N,double Freq, double Delta, double Xeq) {
  if ( (mode>=Dim) && (mode<0) ) { return EXIT_FAILURE; }
  if (Basis[mode] != 0 ) { return EXIT_FAILURE; }
  NGrid[mode] = N;
...
int DVR::AddExpBasis(int mode, int N,double Freq, double Delta, double Xeq) {
  if ( (mode>=Dim) && (mode<0) ) { return EXIT_FAILURE; }
  if (Basis[mode] != 0 ) { return EXIT_FAILURE; }
  NGrid[mode] = N;
